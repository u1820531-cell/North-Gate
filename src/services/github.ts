...
export class GitHubSyncService {
  ...
  /**
   * Translates a Note's metadata and markdown structure to the target folder scheme:
   * notes/YYYY/MM/YYYY-MM-DD/filename.md
   */
  ...
  /**
   * Standardizes serializing our relational Note to flat standard MD files with frontmatter.
   */
  ...
  /**
   * Pushes a single file change directly upstream.
   */
  ...
  /**
   * Loops through changed notes and pushes them, using the Project context if attached.
   */
  public static async syncPendingNotes(): Promise<{ success: number; failed: number }> {
    const results = { success: 0, failed: 0 };
    const notes = await db.notes.toArray(); // In production, query a delta table for modified records
